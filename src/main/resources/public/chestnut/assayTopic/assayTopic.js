$(function() {
    ...
    function submitFun() {
        ...
        var dataJson = {};
        dataJson["token"] = myLocal.getItem('token');
        ...
        $.ajax({
            ...
            // {
            //     "token": myLocal.getItem('token'),
            //     "patientAge": myLocal.getItem('userInfo').patientAge,
            //     "bloodRoutineAnswer": bloodRoutineAnswer == '' ? null : bloodRoutineAnswer, //血常规量表结果
            //     "digitalRectalAnswer": digitalRectalAnswer == '' ? null : digitalRectalAnswer, //前列腺指诊量表结果
            //     "expressedProstaticSecretionAnswer": expressedProstaticSecretionAnswer == '' ? null : expressedProstaticSecretionAnswer, //前列腺按摩液检查量表
            //     "specificAntigenAnswer": specificAntigenAnswer == '' ? null : specificAntigenAnswer, //特异性抗原量表结果
            //     "ultrasonographyBAnswer": ultrasonographyBAnswer == '' ? null : ultrasonographyBAnswer, //B超量表结果
            //     "urineFlowRateAnswer": urineFlowRateAnswer == '' ? null : urineFlowRateAnswer, //尿流率量表结果
            //     "urineRoutineAnswer": urineRoutineAnswer == '' ? null : urineRoutineAnswer, //尿常规检查量表结果
            //     "bloodRoutineInspectTime": bloodRoutineInspectTime == '' ? null : bloodRoutineInspectTime, //血常规检查时间
            //     "digitalRectalInspectTime": digitalRectalInspectTime == '' ? null : digitalRectalInspectTime, //前列腺指诊检查时间
            //     "expressedProstaticSecretionInspectTime": expressedProstaticSecretionInspectTime == '' ? null : expressedProstaticSecretionInspectTime, //前列腺按摩液检查时间
            //     "specificAntigenInspectTime": specificAntigenInspectTime == '' ? null : specificAntigenInspectTime, //特异性抗原检查时间
            //     "ultrasonographyBInspectTime": ultrasonographyBInspectTime == '' ? null : ultrasonographyBInspectTime, //B超检查时间
            //     "urineFlowRateInspectTime": urineFlowRateInspectTime == '' ? null : urineFlowRateInspectTime, //尿流率检查时间
            //     "urineRoutineInspectTime": urineRoutineInspectTime == '' ? null : urineRoutineInspectTime, //尿常规检查时间
            // }
            success: function(data) {
                console.log(data)
                if (data.code == '20000') {
